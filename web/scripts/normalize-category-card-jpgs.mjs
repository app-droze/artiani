...
const LOG_PREFIX = "[normalize-category-card-jpgs]";

const CATEGORY_CARD_TARGETS = [
  {
    target: "category-runners-card.jpg",
    sourceCandidates: [
      "category-runner-card.jpg",
      "category-runner-card.jpeg",
    ],
    pngToDelete: "category-runners-card.png",
  },
  {
    target: "category-scarves-card.jpg",
    sourceCandidates: [
      "category-scarves-card.jpg",
      "category-scarvs-card.jpg",
      "category-scarvs-card.jpeg",
    ],
    pngToDelete: "category-scarves-card.png",
  },
];
...
const main = async () => {
  ...
  for (const asset of CATEGORY_CARD_TARGETS) {
  ...
  const missingCanonicalJpgs = CATEGORY_CARD_TARGETS
  ...
  if (missingCanonicalJpgs.length > 0) {
    ...
    for (const asset of CATEGORY_CARD_TARGETS) {
      if (!existingNames.has(asset.pngToDelete)) {
        console.log(`SKIP    png missing ${asset.pngToDelete}`);
        deleteSkipped += 1;
        continue;
      }

      if (dryRun) {
        console.log(`WOULD   delete ${asset.pngToDelete}`);
        continue;
      }

      const { error } = await storage.remove([asset.pngToDelete]);

      if (error) {
        console.log(`FAILED  delete ${asset.pngToDelete} (${error.message})`);
        failed += 1;
        continue;
      }

      existingNames.delete(asset.pngToDelete);
      console.log(`DELETED ${asset.pngToDelete}`);
      deleted += 1;
    }
  }
