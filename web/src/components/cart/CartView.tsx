...
import { CheckoutView, type CheckoutSubmitResult } from "@/src/components/checkout/CheckoutView";
import type { Dictionary } from "@/src/i18n/getDictionary";
import { t } from "@/src/i18n/getDictionary";
import type { Locale } from "@/src/i18n/locales";
import { getCartDisplayProductTypeLabel, getCartDisplayTitle, writeStoredCart } from "@/src/lib/cart";
...
export const CartView = ({ lang, dict }: CartViewProps) => {
  ...
  const [submittedOrder, setSubmittedOrder] = useState<CheckoutSubmitResult | null>(null);
  ...
  if (submittedOrder) {
    return <CheckoutView lang={lang} dict={dict} persistedSubmitResult={submittedOrder} />;
  }
  ...
  return (
    <section className="mx-auto flex w-full max-w-5xl flex-col gap-4 px-4 py-5 sm:px-6 sm:py-6 md:py-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link
          href={`/${lang}/catalogue`}
          className="text-sm text-black/58 underline underline-offset-4 transition-opacity hover:opacity-70"
        >
          {t(dict, "cart.continueShopping")}
        </Link>
      </div>
      ...
      <CheckoutView lang={lang} dict={dict} onSubmitted={setSubmittedOrder} />
    </section>
  );
};
