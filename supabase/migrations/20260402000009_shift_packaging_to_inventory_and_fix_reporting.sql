comment on table public.order_packaging_usage is
  'Deprecated: packaging is now tracked through inventory and operating expense flows instead of being allocated per order.';

comment on table public.order_misc_costs is
  'Per-order exceptional adjustments such as remakes, rush handling, or other one-off order-specific costs. Packaging should be tracked through inventory and overall expenses instead.';
...
select
  ...
  packaging.unit_cost,
  packaging.currency,
  'Backfilled from packaging_catalog on 2026-04-02 after packaging costs moved out of per-order reporting.'
from public.packaging_catalog as packaging
where not exists (
  select 1
  from public.inventory_items as inventory_items
  where inventory_items.packaging_catalog_id = packaging.id
     or inventory_items.code = packaging.code || '_packaging'
);
...
misc_totals as (
  select
    order_id,
    sum(amount) as misc_cost_amount
  from public.order_misc_costs
  group by order_id
)
select
  orders.id as order_id,
  coalesce(delivery_totals.delivery_cost_amount, orders.shipping_amount, 0) as delivery_cost_amount,
  coalesce(misc_totals.misc_cost_amount, 0) as misc_cost_amount,
  coalesce(delivery_totals.delivery_cost_amount, orders.shipping_amount, 0)
    + coalesce(misc_totals.misc_cost_amount, 0) as fulfillment_cost_amount,
...
left join misc_totals
  on misc_totals.order_id = orders.id;

comment on view public.reporting_order_fulfillment_costs_v1 is
  'Order-level courier and exceptional per-order costs used to compute contribution profit. Packaging is tracked through inventory and operating expenses.';
...
select
  ...
  fulfillment_costs.misc_cost_amount as order_misc_cost_amount,
...
comment on view public.reporting_order_line_item_profit_v1 is
  'Reporting order lines with item cost rules plus courier and exceptional per-order costs. Packaging is handled outside per-order profit allocation.';
...
line_totals as (
  select
    ...
    sum(allocated_delivery_cost_amount) filter (where has_cost_rule) as known_fulfillment_cost_amount,
    sum(allocated_misc_cost_amount) filter (where has_cost_rule) as known_misc_cost_amount,
...
select
  ...
  coalesce(line_totals.known_misc_cost_amount, 0) as known_misc_cost_amount,
...
comment on view public.reporting_daily_sales_v1 is
  'Daily sales rollup with known COGS, courier cost, exceptional per-order costs, and resulting order contribution profit.';
...
comment on view public.reporting_monthly_finance_v1 is
  'Monthly finance rollup combining revenue, COGS, courier cost, exceptional per-order costs, and operating expenses. Packaging stays outside per-order allocation.';
