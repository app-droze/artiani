...
begin
  ...
  if current_time >= event_row.ends_at then
    return query
  ...
  if event_row.ends_at - current_time <= interval '10 minutes' then
...
comment on function public.place_auction_bid(uuid, text, text, numeric) is
  'Safely validates and inserts one Artiani auction bid under a row lock on auction_events, including paid-order eligibility checks and 10-minute anti-sniping extension.';
