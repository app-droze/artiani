create table if not exists public.order_misc_costs (
  ...
  constraint order_misc_costs_amount_check
    check (amount >= 0)
);

create index if not exists order_misc_costs_order_idx
  on public.order_misc_costs (order_id);

comment on table public.order_misc_costs is
  'Per-order manual add-on costs such as extra packaging, gift inserts, rush handling, or other fulfillment adjustments.';
...
select
  ...
  coalesce(packaging_totals.packaging_cost_amount, 0)
...
comment on view public.reporting_order_fulfillment_costs_v1 is
  'Order-level packaging, courier, and manual additional costs used to compute true order contribution profit.';
...
comment on view public.reporting_order_line_item_profit_v1 is
  'Reporting order lines with item cost rules plus packaging, courier, and manual additional costs.';
...
comment on view public.reporting_daily_sales_v1 is
  'Daily sales rollup with known COGS, fulfillment cost, and resulting order-level profit.';
...
comment on view public.reporting_monthly_finance_v1 is
  'Monthly finance rollup combining revenue, COGS, fulfillment cost, and operating expenses.';
